...
def _is_blackout(symbol: str) -> bool:
    ...
    for item in [x for x in bl.split(";") if x.strip()]:
        ...
        if k == sym: return True
    return False

def equities_momentum_signal(symbol: str, timeframe: str, df: pd.DataFrame) -> Optional[dict[str, Any]]:
    ...
    if long_ok:
        return {"side":"LONG","entry":price,"sl_pips":sl_pp,"tp_pips":tp_pp,"note":"equities_momentum",
                "debug":{"price":price,"ema_fast":ef,"ema_slow":es,"rsi":r,"eps":eps,"regime":regime}}
    if short_ok:
        return {"side":"SHORT","entry":price,"sl_pips":sl_pp,"tp_pips":tp_pp,"note":"equities_momentum",
                "debug":{"price":price,"ema_fast":ef,"ema_slow":es,"rsi":r,"eps":eps,"regime":regime}}
    return {"side":"","note":"no_trade","debug":{"price":price,"ema_fast":ef,"ema_slow":es,"rsi":r,"eps":eps,"regime":regime},
