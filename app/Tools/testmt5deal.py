...
from_date = datetime(2020, 10, 9)
to_date = datetime.now()
# get deals for symbols whose names contain "USD" within a specified interval
deals = mt5.history_deals_get(from_date, to_date, group="*USD*")
if deals == None:
    print(f'No deals with group="*USD*", error code={mt5.last_error()}')
elif len(deals) > 0:
    print(f'history_deals_get({from_date}, {to_date}, group="*USD*")={len(deals)}')
