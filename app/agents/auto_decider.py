...
# Pull bars (returns a pandas DataFrame; empty df on failure)
from app.market.data import get_rates

# Strategies
from app.strategies.momentum import momentum_signal  # FX & generic
try:
    from app.strategies.xau_momentum import xau_momentum_signal  # gold-specific
except Exception:
...
def _symbol_alias(symbol: str) -> str:
    # Allow mapping base name -> broker symbol, e.g.
    # SYMBOL_ALIAS_EURUSD=EURUSD-ECNc
...
def _per_symbol_lots(symbol: str, fallback: float) -> float:
    ...
    if val is not None:
        try:
            ...
            pass
    try:
        return float((os.getenv("MT5_DEFAULT_LOTS") or str(fallback)).split("#", 1)[0].strip())
    except Exception:
        return fallback
...
def _log_decider(msg: str) -> None:
    # single place to gate verbosity
...
def decide_signal(symbol: str, timeframe: str = "M15", agent: str = "auto") -> dict[str, Any]:
    """
    Returns a signal dict or a 'no trade' dict, *always* with debug/why when possible.
    """
    ...
    try:
        if strategy_id == "MOMENTUM" and is_gold and xau_momentum_signal is not None:
    ...
    # --- Size (lots) fallback if strategy didn't provide size ---
    if "size" not in sig or sig.get("size") is None:
        sig["size"] = _per_symbol_lots(routed_symbol, 0.01)

    # --- SL/TP fallback (FX vs XAU) ---
    has_sl = sig.get("sl_pips") is not None
    has_tp = sig.get("tp_pips") is not None
    if is_gold:
        if not has_sl:
            sig["sl_pips"] = float(_env_int("MOMENTUM_XAU_SL_PIPS", 300))
            sig.setdefault("debug", {})["sl_source"] = "default"
        if not has_tp:
            sig["tp_pips"] = float(_env_int("MOMENTUM_XAU_TP_PIPS", 600))
            sig.setdefault("debug", {})["tp_source"] = "default"
    else:
        if not has_sl:
            sig["sl_pips"] = float(_env_int("MOMENTUM_FX_SL_PIPS", 50))
            sig.setdefault("debug", {})["sl_source"] = "default"
        if not has_tp:
            sig["tp_pips"] = float(_env_int("MOMENTUM_FX_TP_PIPS", 120))
            sig.setdefault("debug", {})["tp_source"] = "default"

    sig.setdefault("symbol", routed_symbol)
    sig.setdefault("timeframe", timeframe)
    sig.setdefault("agent", agent)
    sig.setdefault("note", sig.get("note") or ("xau_momentum" if is_gold else "momentum"))
